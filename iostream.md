...
```cpp
// Программа, возвращающая реузльтат сложения, вычитания,
// умножения, и деления одного числа на другое.
#include<iostream>
using namespace std;
int main() {
    double a, b;
...
Также в C++ можно производить операции со скобками. Также как и в обычной арифметике они меняют порядок действий и действия в скобках имеют больший приоритет. Например, эта программа считает периметр прямоугольника с целыми сторонами, и если бы не стояли скобки, то вместо (a + b) * 2 возвращалось бы a + b*2.
