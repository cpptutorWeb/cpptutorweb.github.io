...
На протяжении курса мы не будем использовать сторонние библиотеки, поэтому, чтобы каждый раз не писать `std::` можно вначале кода написать строку `using namespace std;`. Теперь отпадает необходимость указывать принадлежность имени к пространству имен `std`.
